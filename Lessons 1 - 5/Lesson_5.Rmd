...
```{r filtering accoding to visual analysis of the plots both}
...
## Normalize and find variable genes for each data set
```{r normalize and find variable genes both}
...
And normalise merged dataset:
```{r normalize_merge}
Seurat_AD_WT <- NormalizeData(Seurat_AD_WT)
```
...
We will first proceed WITHOUT data integration for comparative reasons:
...
Lets compare the Variable genes between the merged and single data sets
```{r, compare VST all}
# Identify the 20 most highly variable genes
top20 <- head(VariableFeatures(Seurat_AD_WT), 20)
top20_WT <- head(VariableFeatures(Seurat_WT), 20)
top20_AD <- head(VariableFeatures(Seurat_AD), 20)
print(paste0("Merged data: ", paste(sort(top20),collapse=",")))
print(paste0("WT data: ", paste(sort(top20_WT),collapse=",")))
print(paste0("AD data: ", paste(sort(top20_AD),collapse=",")))

```

Lets look at the overlap of 1000 genes:

```{r, compare VST overlap}
vec_list <- list(Merge = VariableFeatures(Seurat_AD_WT), 
                 WT =  VariableFeatures(Seurat_WT), 
                 AD = VariableFeatures(Seurat_AD))
# Create an overlap matrix
overlap_matrix <- outer(vec_list, vec_list, Vectorize(function(x, y) length(intersect(x, y))))

print("Raw numbers")
print(overlap_matrix)
print("Percent overlap")
print(overlap_matrix/2000*100)
```
...
The function IntegrateLayers() corrects technical batch effects across multiple separate data layers (samples/batches) to align shared cell types.
...
```
```{r umapplot ckuster, fig.height=8, fig.width=8}
N_clusters <-length(unique(Idents(Seurat_AD_WT)))
# note that you can set `label = TRUE` or use the LabelClusters function to help label individual clusters
...
What does the CCA integration do?
```{r umapplot compare, fig.height=8, fig.width=8}
# note that you can set `label = TRUE` or use the LabelClusters function to help label individual clusters
plot2 + plot4 
```

## Re-join layers after integration
JoinLayers() simply combines separate sample layers back into a single unified layer without doing any mathematical batch correction. Concatenates individual sample matrices back into a single default counts or data, used after successful integration.
```{r join layers}
...
```{r features look,fig.height = 7, fig.width = 10}
FeaturePlot(
  object = Seurat_AD_WT,
  features = c(
    "Slc1a3", #Astrocytes
...
Compare the results of FindMarkers and FindConservedMarkers for a given cluster.
Then contrast this with FindMarkers comparingWT and AD for a given cluser.
```{r, fig.height = 7, fig.width = 14}
