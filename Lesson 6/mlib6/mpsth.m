function [psth, trialspx, ForError] = mpsth(spxtimes,trigtimes,varargin)
...
ForError = zeros(size(psth,1),length(trigtimes));
Cont_Error = 0;
  ...
  elseif binsz>1
    try
      for j = 1:numel(trialspx{i})
        ...
        ForError(floor(trialspx{i}(j)/binsz+pre/binsz+1),Cont_Error) = ForError(floor(trialspx{i}(j)/binsz+pre/binsz+1),Cont_Error) + 1;
      end
    end
  end
end
...
if fr==1
  ...
  for i = 1:length(trigtimes)
      ForError(:,i) = (1/binsz)*1000*ForError(:,i);
  end
end
%% remove time base
if tb==0
...
elseif chart==2
  ...
  for i = 1:numel(trialspx)
    plot(timevec,rastmat(i,:)*i,'Color','r','Marker','.','MarkerSize',1,'LineStyle','none'),hold on
  end
